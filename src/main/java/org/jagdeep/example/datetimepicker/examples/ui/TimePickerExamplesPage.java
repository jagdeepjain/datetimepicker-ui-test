...
public class TimePickerExamplesPage {
    ...
    protected By dateTimePickerHourMinuteLocator = By
            .className("ui-slider-handle");
    protected By dateTimePickerDoneLocator = By
            .className("ui-datepicker-close");
    ...
    public TimePickerExamplesPage setDateTime(int month, int selectedDate,
        ...
        // set date
        int size = dateTimePicker.findElements(dateTimePickerDateLocator)
                .size();
        for (int i = 1; i <= size; i++) {
            String text = dateTimePicker
                    .findElements(dateTimePickerDateLocator).get(i).getText();
            ...
            if (!" ".equals(text)) {
                ...
                if (dateTimePickerDateLocatorText == selectedDate) {
                    dateTimePicker.findElements(dateTimePickerDateLocator)
                            .get(i).click();
                    break;
                }
            }
        ...
        for (int i = 1; i <= startHour; i++) {
            dateTimePicker.findElements(dateTimePickerHourMinuteLocator).get(0)
                    .sendKeys(Keys.ARROW_RIGHT);
        }
        // set minute
        for (int i = 1; i <= startMinute; i++) {
            dateTimePicker.findElements(dateTimePickerHourMinuteLocator).get(1)
                    .sendKeys(Keys.ARROW_RIGHT);
        }
        // clicking on done button
        dateTimePicker.findElement(dateTimePickerDoneLocator).click();
        
        // returning on the same page as date picker does not navigate to any
        // page
        return this;
    }
